...
- **Base16-style theming.** Shipped early in v1.7 (two built-in themes) and expanded in v1.8 to 17 total: `catppuccin-mocha` (default), `classic-mac`, plus 15 base16/community palettes — Dracula, Gruvbox Dark Medium, Nord, Tokyo Night, One Dark, Solarized (Dark + Light), Tomorrow Night, Monokai, Rose Pine, Everforest Dark, Kanagawa, GitHub (Dark + Light), and Catppuccin Latte. Each theme is a single CSS file under `internal/web/assets/themes/` that overrides Catppuccin-named vars under a `[data-theme="<name>"]` selector. `<Space>t` opens a picker overlay with live preview; `?theme=X` (URL) and `localStorage.snackpageTheme` (persisted) both select. The proper base16 vocabulary refactor (`--base00..--base0F` + semantic aliases) remains future work — today's themes piggyback on the Catppuccin-flavored variable names. User themes from `$XDG_CONFIG_HOME/snackpage/themes/` also remain a v3+ addition. See `ARCHITECTURE.md` §8.
